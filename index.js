...
async function run() {
  try {
    ...
    // recommends //
    app.get("/api/v1/recommends", async (req, res) => {
      const recommends = recommendsCollection.find();
      const result = await recommends.toArray();
      res.send(result);
    });
    // recommends //
    ...
    app.post("/api/v1/payments", async (req, res) => {
      ...
      // send user email after payment confirmation //

      mg.messages
        .create(process.env.MAIL_GUN_DOMAIN, {
          from: "Mailgun Sandbox <postmaster@sandbox730475af892346bcbd44d8cd86fddf81.mailgun.org>",
          to: ["foyez1435@gmail.com"],
          subject: "Thanks for your ordering !",
          text: "Testing some Mailgun awesomness!",
          html: `<div>
            <h1> Thank your for order </h1>
            <h3> Your transaction Id : <strong> ${payment.transactionId} </strong> </h3>
            <p> We would like to get feedback about food </p>
          </div>`
        })
     ...
     * 3. for every item in the menu collection that found for a payment document
     * ----------------
     */
